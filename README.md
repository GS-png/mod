...
- `src/`：当前实现源码。
- `docs/`：设计文档、UI 设计源文件、外部 API 参考、参考素材、启动自检、轮回脚本、存档回归、成长回归、压力测试、发布说明和发布前检查表。
- `Locales/`：中英文文本键。
- `default_config.json`：默认玩法参数。
- `mod.json`：模组身份和基础元数据。

## 推荐阅读顺序
1. 先看 `docs/设计/EraWheel_Redesign.md`。它负责讲玩法总览。
2. 再看 `docs/设计/EraWheel_实现口径.md`。它负责讲系统边界和实现口径。
3. 想查具体参数时，再看 `docs/设计/ERRE附属文档/系统参数总表.md`。
4. 准备联调时，直接进 `docs/验证与发布/` 按 `EW-114` 到 `EW-121` 的顺序走。
...
3. 进实机前，先看 `docs/验证与发布/EW-114_启动自检清单.md`。
