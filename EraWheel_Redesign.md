...
| 属性ID | 简单说明 | 备注 |
|---|---|---|
| `personality_aggression` | 文明好战倾向 | 隐藏 |
| `personality_administration` | 文明内政倾向 | 隐藏 |
| `personality_diplomatic` | 文明外交倾向 | 隐藏 |
| `personality_rationality` | 文明理性倾向 | 隐藏 |
| `diplomacy` | 文明外交能力 | - |
| `warfare` | 文明军事能力 | - |
| `stewardship` | 文明治理/管理能力 | - |
| `intelligence` | 文明智略/研究能力 | - |
| `army` | 文明军力规模/动员能力 | - |
| `cities` | 城市发展能力/城市上限相关 | - |
| `bonus_towers` | 防御塔相关加成 | - |
| `loyalty_traits` | 文明忠诚特质倾向 | - |
| `loyalty_mood` | 文明忠诚/士气情绪 | - |
| `opinion` | 文明对外好感/评价 | - |
...
| 纪元遗产（Legacy） | 轮回结算后给予文明的永久增益（跨轮回继承） |
...
| 纪元遗产 | 轮回的“记忆” | 轮回结算数据 | 文明/魔王遗产 | [2.3](#s23) |
...
| **纪元遗产** | 轮回结算后文明+英雄+魔王获得遗产：文明侧以“工艺/研究”解锁装备与能力，魔王侧以“征服/魔能”强化军势（均可配置） |
...
- 生成点与主要文明/王城保持安全距离，避免“出生即撞城”或卡位
- 据点逐渐扩大，污染向外扩散
- 第一批将领开始激活（通常 1-2 个）
- 尚不生成军团（或生成极少）
...
- 可通过两种方式之一胜利：
  ...
  2. **仪式封印**：友方单位在遗迹范围内完成“占据”（先清理完遗迹范围内的魔王势力，且人数达到阈值）即可**触发占领状态**；触发后不要求持续保持人数，除非触发中断条件。  
     ...
     （占领人数与判定方式/清场要求/占领遗迹数量/进度速度/中断条件可配置，见 [7.1.11](#s71)）
  3. **遗迹争夺驱动（让双方会去打）**：
     - **战场热点**：系统定期从“魔王阵营”和“附近文明”各抽取一小队，给一次性 `goTo(遗迹附近)` 指令，把遗迹变成真实战场热点（不接管 AI 行为树）。  
     - **占领收益**：占领遗迹后，遗迹范围内友军获得小幅增益，敌军获得小幅削弱，促使双方主动争夺。  
...
| 全盛 | >70% | 所有技能 | ✅ 每5年（加速） | 按轮回 | 法力恢复 +50%（可配，见 7.1.2），军团加速 ×2（见 7.1.3） |
| 衰弱 | <30% | 被动仅 | ❌ | ×0.5（可配，见 7.1.2） | 优先逃离，部分将领撤退/失控 |
| 再封印 | 0% | ❌ | ❌ | 0 | 轮回结算，状态重置 |
...
每次轮回结算后，**文明、英雄、魔王都会获得各自的“纪元遗产”**。  
文明遗产影响文明与英雄；魔王遗产影响魔王阵营。  
...
| 遗产类型 | 效果示例 | 档位规则 |
|---------|---------|--------|
| **工艺遗产（文明）** | 解锁更高阶装备/材料池；英雄与精英单位可获得更高品质装备 | 档位 1-10：随档位扩展装备池 |
| **研究遗产（文明）** | 解锁“封印研究/破魔锻造”等能力与英雄技能池 | 档位 1-10：随档位扩展技能池 |
| **军事遗产（文明）** | 对魔王阵营单位攻击 +10%，防御 +10%（基础档位） | 档位 1-10：加成幅度递减（档位越高增量越小） |
...
**口径说明**：战斗数值强化主要归魔王遗产；文明侧以“工艺解锁/装备强化/英雄传承”为主，避免变成“文明也直接数值碾压”的不合理体验。  
**档位规则说明**：
...
**工艺遗产（文明）档位**（解锁装备池，全部为 MOD 自定义装备，带自定义外观/特效）：
- 1 层：基础工坊  
...
装备生成时写入“制造文明/制作者”标记，用于叙事与追溯。

**研究遗产（文明）档位**（解锁技能池/能力，全部为 MOD 自定义技能）：
- 1 层：基础研究  
...
- `城市规模分` = `clamp01(城市数 / 5)`  
- `人口规模分` = `clamp01(总人口 / 5000)`  
- `军力规模分` = `clamp01(战士数 / 200)`  
- `工艺规模分` = `clamp01(所有城市建筑总数 / 100)`  
...
// 文明遗产：对 Kingdom 添加 KingdomTrait（影响文明与英雄）
...
// 在文明单位 trait_era_legacy_military.action_attack_target：
// if (target.hasTrait("trait_era_demon_faction")) target.changeHealth(-self.damage * 0.10f);
// 在文明单位 trait_era_legacy_military.action_get_hit：
...
- 生成地点：默认在魔王据点附近成团生成并集结；若据点不存在则在魔王附近或世界边缘可行走地块生成，再向最近文明推进
...
- **激活**：降临阶段开始激活；将领使用独立 `ActorAsset` 模板生成（不改造现存单位）；激活数量与轮回成长参考 `3.2 将领成长`（上限 6 个同时激活）。
...
文明通过与魔王战斗，逐步提升抗魔等级（上限可配置），形成“越战越强”的正反馈。  
...
**实现要点（可直接实现）**：
- 抗魔等级是“文明维度”数据：用 `KingdomTrait` 表现（显示/持久化），并在单位生成/出生时继承其文明 trait 的特点属性加成。
- 加成只作用于文明侧单位的“特点属性”，不改敌方生成与底层伤害公式，保持稳定可控。
...
- 当文明满足繁荣/危机阈值（人口、城市、战损等，**二选一/任一满足即可**；**统计范围=该文明自身**）时，系统从该文明现存单位中挑选 1 名候选并“晋升”为命定英雄：
  - 繁荣/危机阈值默认见 [7.1.4](#s71)（与 2.1.2 的繁荣度阈值**可独立调整**，需要时可手动对齐）
  - 优先：高等级/高击杀/寿命时间长/基础属性高的单位
  - 若没有合格候选：允许直接生成 1 名“命定英雄模板单位”（可选）

**评分规则（可配置）**：
- **候选池过滤**：同文明、已成年、存活、非魔王/将领/军团、未被标记为英雄。
...
- 额外加成：`max_mana_bonus`、`combat_mana_regen_bonus`
...
- 并与所有王国保持战争关系（共同敌人）。
- **敌对范围（明确）**：除魔王阵营之外，**所有单位与生物都视为敌人**（文明单位/英雄/中立生物/野兽等一律可攻击），符合“毁灭世界”的设定。
- 讨伐关系的落地方式：
  ...
  4) **目标选择**：魔王阵营单位在选敌时，以“非魔王阵营单位”为目标（包含中立/野兽/文明/英雄）。
...
> 本设计不使用传统冷却条，使用游戏内置“法力（Mana）”作为技能资源；触发频率统一规则见「[统一口径](#conventions)」与「[7.1.7](#s71)」。
...
说明：以下 S1 消耗与频率遵循统一口径（见 [7.1.7](#s71)），不再逐条重复标注。
...
| 栏目 | 玩家能看到什么 | 玩家能做什么 |
|---|---|---|
| **总览** | 轮回、阶段、封印、魔王与文明概况 | 只读为主、少量快捷按钮 |
| **战局** | 魔王/将领/军团/英雄/文明列表与定位 | 筛选、定位、关注 |
...
> 高级/开发模式可额外显示：**魔王管理 / 文明状态 / AI 控制 / 事件管理 / 轮回历史**（与开发文档一致）。
...
**玩法核心卡示例**
> “这是一个**轮回制史诗玩法**：魔王会多次苏醒，你需要培养文明和英雄，在封印战中击败魔王；每轮结束都会获得永久遗产，让下一轮更强。”
...
**遗产一览（玩家版）**
- **文明遗产**：工艺 / 研究 / 军事 / 传承  
  - 例：工艺 = 解锁装备池；研究 = 解锁技能；军事/传承 = 增益英雄与文明
- **魔王遗产**：征服 / 军备 / 魔能 / 领域  
...
**一句话理解**
> 魔王会一轮又一轮地苏醒，你需要培养文明和英雄，在封印战中击败魔王；每轮结束都会获得永久遗产，让下一轮更有优势。
...
说明：本附录中仍沿用原 5.x/6.x/7.x 小节编号，便于历史引用；新增 E/F 为技能库（原 2.2.4 / 2.4.2.3），新增 G 为可用属性字段速查。
...
| `World.world.units/kingdoms/cities/wars` | 获取单位/文明/城市/战争列表（运行态数据入口） | ✅ 确认 | `docs/api/Assembly-CSharp.md`（`MapBox.units/kingdoms/cities/wars` 为 public） |
...
| `Kingdom.countCities/countUnits/countWarriors` | 文明规模/兵力统计 | ✅ 确认 | `docs/api/Assembly-CSharp.md` |
...
- **命名与叙事标记**：装备/技能可写入“制造文明/制作者/研发年份”，用于叙事可追溯。
...
| `trait_era_mix_mystic` | 奥术亲和 | 其他 | 法力上限 +80；战斗回蓝 +1/触发；伤害 +10% | 可随机 |
...
| 参数 | 默认值 | 说明 |
|------|-----|------|
| `世界现存人口阈值` | 5000 | 世界总人口达到此值时，可从“预发展”进入“预兆”（第 1 轮回用，人口或城市满足其一即可） |
| `世界现存城市阈值` | 20 | 世界城市达到此值时，可从“预发展”进入“预兆”（与人口阈值二选一） |
| `预发展_检查间隔` | 10 年 | 系统每隔多久检查一次阈值（越短越快进入预兆） |
| `预兆_持续年数` | 30 | 预兆阶段持续多久（越长 = 魔王更晚苏醒） |
| `预兆_事件间隔` | 3-5 年 | 每次事件后随机间隔 3~5 年再出现（随机间隔） |
| `苏醒_持续年数` | 50 | 苏醒阶段持续多久（魔王已现身但未全面入侵） |
| `苏醒_初始强度比例` | 30% | 苏醒初始强度 = 满强度 × 该比例 |
| `降临_持续年数_最短` | 100 | 降临阶段最短持续时间 |
| `降临_持续年数_最长` | 200 | 降临阶段最长持续时间（到期会进入衰弱） |
| `衰弱_持续年数` | 20 | 衰弱阶段持续多久（越短越快进入封印战） |
| `封印战_持续年数` | 50 | 封印战窗口持续时间 |
| `封印战_回退_HP阈值` | 70% | 魔王回血到此比例会退出封印战，回到降临 |
| `封印_初始强度` | 100% | 封印起始强度，影响阶段推进节奏 |
| `封印_衰减速率` | 1%/年 | 封印每 1 年下降多少（设为 ≤0 会自动修正） |
| `封印强度_苏醒阈值` | 20% | 封印低于此值进入“苏醒” |
| `封印强度_降临阈值` | 10% | 封印低于此值进入“降临” |

#### 7.1.2 魔王与多魔王

| 参数 | 默认值 | 说明 |
|------|-----|------|
| 魔王启用默认 | 全部启用 | 默认勾选全部魔王，未勾选的不会出现 |
| 魔王苏醒模式 | random（随机） | 指定 = 只出你勾选的；随机 = 从已启用里抽 |
| 苏醒数量 | 1 | 仅随机模式生效：本轮会出现几个魔王 |
| 全盛期 HP 阈值 | 70% | 血量高于该值进入全盛（技能更频繁、更强） |
| 衰弱期 HP 阈值 | 30% | 血量低于该值进入衰弱，准备封印战 |
| 全盛期法力恢复倍率 | +50% | 全盛期回蓝加成，影响技能释放频率 |
| 衰弱期将领数量倍率 | 0.5 | 衰弱期将领数量 × 该倍数 |
| 轮回成长倍率 | 25%/轮回 | 按基础值叠加：每完成一轮，魔王基础属性 +25% |
| 魔王成长硬上限 | +200% | 基于基础值封顶：最多到基础值 +200% |
| 魔王阵营寿命（lifespan） | 99999 年 | 让魔王/将领/军团基本不会老死 |
| 多魔王互动模式 | random（随机） | 可选：联盟(alliance)/内战(civilwar)/随机(random)（见 2.5.4） |
| 多魔王关系校验间隔 | 3 年 | 每隔多久检查并修正魔王关系 |
| 魔王联盟强度系数 | 0.85 | 联盟模式下单个魔王最终属性整体乘数 |
| 魔王内战强度系数 | 1.00 | 内战模式下单个魔王最终属性整体乘数 |
| 内战最大魔王数 | 5 | 参与内战的魔王上限，超出则随机抽取 |

#### 7.1.3 军团与波次

| 参数 | 默认值 | 说明 |
|------|-----|------|
| `军团_生成间隔` | 10 年 | 每隔多久出一波（≤0 会自动修正） |
| `军团_初始数量` | 30 | 第 1 波生成的单位数量 |
| `军团_强度递增` | 15%/波 | 波次越高强度越高（可为负，表示变弱） |
| `军团_同时上限` | 200 | 场上最多允许的军团数量，超出则不再生成 |
| `军团_全盛加速` | ×2 | 全盛期波次速度 ×2（间隔减半） |
| 波次数量模板 | 1-3 波=30 / 4-6 波=35 / 7-9 波=40 / 10+ 波=50 | 按波段套模板，每一波固定用对应数量 |
| 波次等级增量 | 0 / +1 / +2 / +3 | 按波段给单位 level 直接 +X（生成时生效） |
...
| 参数 | 默认值 | 说明 |
|------|-----|------|
| 抗魔等级范围 | 0-10 | 文明抗魔等级的范围/上限 |
| 抗魔等级_击杀阈值 | 1000 | 累计击杀魔王阵营单位达到该数升 1 级 |
| 抗魔等级_封印加成 | +1.5/次 | 可累计小数（例如 0.5 也保留），达 1 自动进位 |
| 抗魔等级关键档位效果 | Lv1 命中+5%、攻速+5%；Lv2 伤害+5%、生命+5%；Lv3 体力+10%、法力+10%；Lv4 防御+8%、移速+5%；Lv5 暴击率+5%、暴伤+15%；Lv6 伤害+8%、防御+6%；Lv7 射程+10%、投掷射程+10%；Lv8 生命+10%、移速+5%；Lv9 攻速+8%、暴击率+3%；Lv10 战斗熟练+20%、法术熟练+20%、状态触发+10% | 文明侧关键档位说明（特点属性组合；可用字段见附录G） |
| 命定英雄每文明上限 | 1 名 | 每个文明同时最多几个命定英雄 |
| 命定英雄晋升冷却 | 30 年 | 命定英雄死亡后需等待多久才能再晋升 |
| 命定英雄兜底检测间隔 | 5 年 | 系统多久检查一次是否满足晋升条件 |
| 命定英雄晋升_繁荣_人口阈值 | 3000 | 繁荣判定二选一：人口达到此值即可（不要求同时满足城市） |
| 命定英雄晋升_繁荣_城市阈值 | 15 | 繁荣判定二选一：城市达到此值即可（不要求同时满足人口） |
| 命定英雄晋升_危机统计窗口 | 10 年 | 统计近 N 年的危机变化 |
| 命定英雄晋升_危机_人口跌幅 | 20% | 危机判定三选一：近 N 年人口跌幅达到该比例（按窗口起点人口计算） |
| 命定英雄晋升_危机_城市损失数 | 2 | 危机判定三选一：近 N 年损失城市达到该数量 |
| 命定英雄晋升_危机_战损比例 | 30% | 危机判定三选一：近 N 年战死人数占该文明总人口的比例 |
| 英雄候选最低等级 | 3 | 低于该等级不会进入候选池 |
| 英雄候选最低击杀 | 5 | 击杀太少不会进入候选池 |
| 英雄候选最低年龄 | 18 年 | 年龄太小不会被选 |
| 英雄评分权重 | level 0.25 / kills 0.25 / life 0.15 / hp 0.10 / atk 0.10 / lead 0.15 | 各指标占比，数值越大越看重该项 |
| 英雄评分归一化阈值 | level/10、kills/50、life/200、hp/1000、atk/100 | 达到该值记 1 分（1.0），超过也按 1 分 |
| 英雄评分取前 N 随机 | 3 | 从评分前 N 名中随机挑 1 名 |
| 命定英雄基础倍率（damage/armor/health/speed） | +35% | 命定英雄基础属性加成（伤害/防御/生命/移速） |
| 命定英雄基础倍率（attack_speed/critical_*） | +20% | 命定英雄攻速/暴击加成 |
| 命定英雄随机特质数量 | 2 个 | 晋升时额外随机获得的英雄特质数量 |
| 命定英雄随机技能数量 | 1 个 | 晋升时额外随机获得的英雄技能数量 |
| 命定英雄法力上限加成 | +170 | 命定英雄最大法力加成 |
| 命定英雄战斗回蓝加成 | +1/触发 | 命定英雄战斗时额外回蓝幅度 |
| 魔王阵营法力上限加成 | +0 | 魔王/将领/军团最大法力加成 |
| 魔王阵营战斗回蓝加成 | +0/触发 | 魔王/将领/军团战斗回蓝加成 |
...
| 参数 | 默认值 | 说明 |
|------|-----|------|
| 停战复查间隔 | 3 年 | 每隔多久强制结束文明间战争，保持共同讨伐 |
| 共同讨伐触发_降临阶段开关 | 开启 | 开启后魔王进入“降临”就触发停战 |
| 共同讨伐触发_城市摧毁比例 | 20% | 魔王摧毁城市数 / 世界总城市数 达到此值触发停战 |
| 共同讨伐触发_人口阈值 | 500 | 世界人口低于此值触发停战（保底） |

#### 7.1.6 将领与军团倍率

| 参数 | 默认值 | 说明 |
|------|-----|------|
| 每魔王将领数量 | 5 名 | 每个魔王固定带几名将领（5 个定位） |
| 将领激活数量成长 | 起始 2 / 每轮回 +1 / 上限 6 | 同时活跃的将领数量随轮回增长 |
| 将领平均等级成长 | 起始 10 / 每轮回 +3 / 上限 40 | 将领平均等级随轮回提升 |
| 将领平均属性系数 | 跟随魔王 ×0.7 | 将领基础属性 = 魔王属性 × 该系数 |
| 将领定位倍率模板 | 先锋 0.8/1.0/0.6/1.5；坦克 1.5/0.7/1.5/0.7；输出 0.7/1.5/0.5/1.0；辅助 1.0/0.8/1.0/1.0；精英 1.2/1.2/1.2/1.0 | 5 个定位的倍率模板（顺序 = HP/攻/防/速） |
| 军团单位倍率模板 | 先锋 0.7/0.8/0.6/1.3；主力 1.0/1.0/1.0/1.0；精锐 1.6/1.4/1.2/1.0；终极 4.0/2.5/2.0/0.8 | 军团四层单位倍率（顺序 = HP/攻/防/速） |

#### 7.1.7 技能/遗产/装备（大表可调项）

| 参数 | 默认值 | 说明 |
|------|-----|------|
| 主动技能法力消耗（固定-普通） | 5 | 普通主动技能消耗的法力 |
| 主动技能法力消耗（固定-终极） | 10 | 终极技/大招消耗的法力 |
| 主动技能触发上限（普通） | 2 次/年 | 每个普通技能每年最多触发次数 |
| 终极技触发间隔 | 2 年 | 终极技至少隔几年才能再触发 |
| 技能数值（魔王/将领/英雄） | 见对应技能表（附录E/F） | 具体伤害/持续/范围等在技能表里改 |
| 遗产档位与装备/研究技能数值 | 见 2.3.2.1 | 遗产与装备/研究的数值在遗产表里改 |
...
- **随机时机**：单位生成时随机一次；结果写入单位自定义数据，读档不重抽。
...
| 参数 | 默认值 | 说明 |
|------|-----|------|
| `随机倍率_魔王_生命` | 2.00~6.00 | 魔王生成时生命随机倍率范围 |
| `随机倍率_魔王_防御` | 1.80~4.00 | 魔王生成时防御随机倍率范围 |
| `随机倍率_魔王_伤害` | 2.20~5.00 | 魔王生成时伤害随机倍率范围 |
| `随机加成_魔王_暴击率` | 12%~35% | 魔王生成时额外暴击率随机范围 |
| `随机倍率_魔王_移速` | 1.10~1.40 | 魔王生成时移速随机倍率范围 |
| `随机倍率_魔王_攻速` | 1.10~1.40 | 魔王生成时攻速随机倍率范围 |
| `随机倍率_将领_生命` | 1.50~3.50 | 将领生成时生命随机倍率范围 |
| `随机倍率_将领_防御` | 1.30~2.80 | 将领生成时防御随机倍率范围 |
| `随机倍率_将领_伤害` | 1.60~3.50 | 将领生成时伤害随机倍率范围 |
| `随机加成_将领_暴击率` | 10%~25% | 将领生成时额外暴击率随机范围 |
| `随机倍率_将领_移速` | 1.05~1.25 | 将领生成时移速随机倍率范围 |
| `随机倍率_将领_攻速` | 1.05~1.25 | 将领生成时攻速随机倍率范围 |
| `随机倍率_军团_生命` | 1.20~2.00 | 军团单位生成时生命随机倍率范围 |
| `随机倍率_军团_防御` | 1.10~1.80 | 军团单位生成时防御随机倍率范围 |
| `随机倍率_军团_伤害` | 1.20~2.00 | 军团单位生成时伤害随机倍率范围 |
| `随机加成_军团_暴击率` | 6%~18% | 军团单位生成时额外暴击率随机范围 |
| `随机倍率_军团_移速` | 1.00~1.20 | 军团单位生成时移速随机倍率范围 |
| `随机倍率_军团_攻速` | 1.00~1.20 | 军团单位生成时攻速随机倍率范围 |
| `随机倍率_英雄_生命` | 1.60~3.20 | 英雄生成时生命随机倍率范围 |
| `随机倍率_英雄_防御` | 1.30~2.50 | 英雄生成时防御随机倍率范围 |
| `随机倍率_英雄_伤害` | 1.60~3.00 | 英雄生成时伤害随机倍率范围 |
| `随机加成_英雄_暴击率` | 10%~25% | 英雄生成时额外暴击率随机范围 |
| `随机倍率_英雄_移速` | 1.05~1.25 | 英雄生成时移速随机倍率范围 |
| `随机倍率_英雄_攻速` | 1.05~1.25 | 英雄生成时攻速随机倍率范围 |
...
| 每轮回档位提升 | +1 | 每完成一轮回提升的档位数 |
| 新王国档位下限 | 1 | 新建王国最低继承档位 |
| 掌握度刷新间隔 | 10 年 | 新王国隔多少年重新计算掌握度 |
...
| 参数 | 默认值 | 说明 |
|------|-----|------|
| 文化书籍阈值 | 10 | 文化书籍达到该数记满分 |
| 宗教书籍阈值 | 10 | 宗教书籍达到该数记满分 |
| 语言书籍阈值 | 10 | 语言书籍达到该数记满分 |
| 首都书籍阈值 | 5 | 首都书籍达到该数记满分 |
| 全城书籍阈值 | 30 | 全城书籍总数达到该数记满分 |
| 城市规模阈值 | 5 | 城市数量达到该数记满分 |
| 人口规模阈值 | 5000 | 总人口达到该数记满分 |
| 军力规模阈值 | 200 | 战士数量达到该数记满分 |
| 工艺规模阈值 | 100 | 建筑总数达到该数记满分 |

**掌握度权重（合计 1.00）**

| 参数 | 默认值 | 说明 |
|------|-----|------|
| 基础掌握度权重 | 0.20 | 掌握度的基础分占比 |
| 文化书籍权重 | 0.10 | 文化书籍分在掌握度中的占比 |
| 宗教书籍权重 | 0.10 | 宗教书籍分在掌握度中的占比 |
| 语言书籍权重 | 0.05 | 语言书籍分在掌握度中的占比 |
| 首都书籍权重 | 0.10 | 首都书籍分在掌握度中的占比 |
| 全城书籍权重 | 0.05 | 全城书籍分在掌握度中的占比 |
| 城市规模权重 | 0.15 | 城市规模分在掌握度中的占比 |
| 人口规模权重 | 0.10 | 人口规模分在掌握度中的占比 |
| 军力规模权重 | 0.10 | 军力规模分在掌握度中的占比 |
| 工艺规模权重 | 0.05 | 工艺规模分在掌握度中的占比 |

#### 7.1.11 封印遗迹与仪式封印（UI 可调）

| 参数 | 默认值 | 说明 |
|------|-----|------|
| 封印遗迹数量 | 10 | 每次封印战生成多少个遗迹 |
| 占领遗迹数量封印阈值 | 8 | 同时占领达到阈值才开始仪式推进 |
| 仪式占领_人数阈值 | 20 | 遗迹范围内友军人数达到该数才算占领 |
| 仪式占领_人数判定 | 触发式 | 触发式 = 人数达标就开始，不必一直保持 |
| 仪式占领_清场要求 | 开启 | 是否必须先清理遗迹内敌军 |
| 仪式进度速度 | 10%/年 | 仪式进度每年累积多少 |
| 仪式中断条件 | 敌军进入 | 哪些情况会打断仪式 |
| 仪式中断处理 | 暂停 | 打断后如何处理进度（暂停/回退/清零） |
| 仪式回退速度 | 5%/年 | 选择回退时每年倒退多少 |
| 封印胜利模式 | 双通道 | 胜利方式：击杀 / 仪式 / 双通道（任一达成即胜） |
| 遗迹战场热点 | 开启 | 是否定期派小队去争夺遗迹（让战斗集中） |
| 热点调度间隔 | 5 年 | 派遣小队的间隔 |
| 热点派遣_文明人数 | 30 | 每次从文明派出的单位数量 |
| 热点派遣_魔王人数 | 30 | 每次从魔王阵营派出的单位数量 |
| 热点目标策略 | 优先敌占领 | 优先派往“敌方占领”的遗迹 |
| 遗迹占领收益_半径 | 12 | 占领收益生效的范围半径 |
| 遗迹占领收益_友军增益 | 攻防 +10% | 占领方范围内友军加成 |
| 遗迹占领收益_敌军削弱 | 攻防 -10% | 非占领方范围内敌军削弱 |
