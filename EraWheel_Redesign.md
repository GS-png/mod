...
| 英雄评分归一化阈值 | 等级/10、击杀/50、寿命/200、生命上限/1000、攻击/100 | 达到这个值记满分（超过也按满分） |
...
| 参数 | 默认值（UI可调） | 说明 |
|------|-----|------|
| 停战复查间隔 | 3 年 | 多久清一次文明之间的战争，保证停战持续 |
...
| 参数 | 默认值（UI可调） | 说明 |
|------|-----|------|
| 将领激活数量成长 | 起始 2 / 每轮回 +1 / 上限 5 | 同时可上场的将领数量随轮回提升 |
...
| 参数 | 默认值（UI可调） | 说明 |
|------|-----|------|
| `随机倍率_魔王_生命` | 2.00~6.00 | 魔王生命倍率随机范围（生成时一次） |
| `随机倍率_魔王_防御` | 1.80~4.00 | 魔王防御倍率随机范围（生成时一次） |
| `随机倍率_魔王_伤害` | 2.20~5.00 | 魔王伤害倍率随机范围（生成时一次） |
| `随机加成_魔王_暴击率` | 12%~35% | 魔王暴击率额外加成范围（生成时一次） |
| `随机倍率_魔王_移速` | 1.10~1.40 | 魔王移速倍率随机范围（生成时一次） |
| `随机倍率_魔王_攻速` | 1.10~1.40 | 魔王攻速倍率随机范围（生成时一次） |
| `随机倍率_将领_生命` | 1.50~3.50 | 将领生命倍率随机范围（生成时一次） |
| `随机倍率_将领_防御` | 1.30~2.80 | 将领防御倍率随机范围（生成时一次） |
| `随机倍率_将领_伤害` | 1.60~3.50 | 将领伤害倍率随机范围（生成时一次） |
| `随机加成_将领_暴击率` | 10%~25% | 将领暴击率额外加成范围（生成时一次） |
| `随机倍率_将领_移速` | 1.05~1.25 | 将领移速倍率随机范围（生成时一次） |
| `随机倍率_将领_攻速` | 1.05~1.25 | 将领攻速倍率随机范围（生成时一次） |
| `随机倍率_军团_生命` | 1.20~2.00 | 军团单位生命倍率随机范围（生成时一次） |
| `随机倍率_军团_防御` | 1.10~1.80 | 军团单位防御倍率随机范围（生成时一次） |
| `随机倍率_军团_伤害` | 1.20~2.00 | 军团单位伤害倍率随机范围（生成时一次） |
| `随机加成_军团_暴击率` | 6%~18% | 军团单位暴击率额外加成范围（生成时一次） |
| `随机倍率_军团_移速` | 1.00~1.20 | 军团单位移速倍率随机范围（生成时一次） |
| `随机倍率_军团_攻速` | 1.00~1.20 | 军团单位攻速倍率随机范围（生成时一次） |
| `随机倍率_英雄_生命` | 1.60~3.20 | 英雄生命倍率随机范围（生成时一次） |
| `随机倍率_英雄_防御` | 1.30~2.50 | 英雄防御倍率随机范围（生成时一次） |
| `随机倍率_英雄_伤害` | 1.60~3.00 | 英雄伤害倍率随机范围（生成时一次） |
| `随机加成_英雄_暴击率` | 10%~25% | 英雄暴击率额外加成范围（生成时一次） |
| `随机倍率_英雄_移速` | 1.05~1.25 | 英雄移速倍率随机范围（生成时一次） |
| `随机倍率_英雄_攻速` | 1.05~1.25 | 英雄攻速倍率随机范围（生成时一次） |
