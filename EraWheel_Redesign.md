...
**文档定位**
- 这是“目标态设计 + 可实现口径”的总文档：既给策划阅读，也给开发落地。
- 玩法与规则在前，技术实现与 API 在后；长表格默认折叠。

**阅读路线（按目标）**
- **快速理解玩法闭环**：先看「[1.2 游戏循环流程](#s12)」→「[2.1 轮回系统](#s21)」→「[2.4 军团波次系统](#s24)」→「[2.5 文明与英雄系统](#s25)」→「[第四部分：用户交互设计](#p4)」
- **只想了解“是什么”**：优先看每个系统小节的「核心概念 / 阶段流程 / 可配置参数 / 保护机制」
- **只想落地实现“怎么做”**：先看「[第五部分：技术可行性与实现方案](#p5)」，再回头按需查系统细节
- **只查默认值/关键参数**：跳到「[7.1 关键参数默认值速查](#s71)」
- **只看技能/单位**：从「[2.2 魔王状态机系统（含技能表）](#s22)」→「[2.6 将领系统](#s26)」→「[2.7 军团系统](#s27)」进入
- **长表格已折叠**：魔王/将领/军团的技能大表默认折叠，可按需展开

**结构速览（1 句话知道每一部分在讲什么）**
- 第一部分：项目目标与玩法闭环（入口：[项目概述](#p1)）
- 第二部分：核心系统设计与规则（入口：[核心系统设计](#p2)）
- 第三部分：数值与平衡（入口：[游戏数值与平衡](#p3)）
- 第四部分：UI/交互设计（入口：[用户交互设计](#p4)）
- 第五部分：技术实现口径（入口：[技术可行性与实现方案](#p5)）
- 第六部分：版本规划（入口：[版本规划与交付](#p6)）
- 第七部分：参数速查（入口：[关键参数默认值速查](#p7)）

**检索建议**
- 关键词：轮回/阶段/魔王/将领/军团/遗产/封印/技能/波次/CSI
- 想回目录：在任意章节使用「[↩ 目录](#toc)」
...
| 军团波次 | 敌军生成节奏 | 阶段/波次配置/军团库 | 军团生成与规模 | [2.4](#s24) |
| 文明与英雄 | 反哺与对抗系统 | 王国/英雄运行态 | 抗魔等级/英雄晋升 | [2.5](#s25) |
| 将领系统 | 精英单位与指挥层 | 魔王类型/轮回数 | 将领激活与技能 | [2.6](#s26) |
| 军团系统 | 单位库与规格 | 魔王类型/军团配置 | 军团单位与技能 | [2.7](#s27) |
...
- 系统自动生成封印遗迹（自定义建筑）与“仪式区域”（范围检测 + 进度条逻辑）
  ...
  - 生成口径：进入封印战阶段后由 MOD 自动选点，在随机点位**直接生成遗迹建筑**（【需反射】通过反射调用 `World.world.buildings` 上的 `internal BuildingManager.addBuilding(string pID, WorldTile pTile, ...)`）；封印后统一移除这些建筑（`World.world.buildings.removeObject(building)`）
- 可通过三种方式之一胜利：
  1. **击杀封印**：直接击杀魔王
...
| `繁荣度_检查间隔` | 10 年 | 每隔 N 年检查一次繁荣度 |
| `预兆_持续年数` | 30 | 预兆持续时间 |
| `预兆_事件间隔` | 3-5 年 | 预兆事件触发频率 |
| `苏醒_持续年数` | 50 | 苏醒持续时间（范围 30-100） |
| `苏醒_初始强度比例` | 30% | 苏醒期魔王强度占最大强度比例 |
| `降临_持续年数_最短` | 100 | 最短入侵时间 |
| `降临_持续年数_最长` | 200 | 最长入侵时间 |
| `衰弱_持续年数` | 20 | 衰弱持续时间 |
| `封印战_持续年数` | 50 | 决战窗口持续（范围 50-100） |
| `封印_初始强度` | 100% | 初始封印强度（可超过 100%） |
| `封印_衰减速率` | 5%/10年 | 衰减速度（可超过 100%） |
| `封印强度_苏醒阈值` | 20% | 低于该值进入苏醒阶段 |
| `封印强度_降临阈值` | 10% | 低于该值进入降临阶段 |
...
> 统一规则：每个魔王 **1 个被动（P0）+ 6 个技能（S1-S6）**；数值可配置；触发与存储口径见「[文档约定](#conventions)」。  
> 主动技能默认用法力（普通技能 5 点、终极技 10 点），不使用传统冷却条，改为频率限制（普通每年最多 2 次 / 终极每 2 年最多 1 次），可在配置中统一调整。
...
- 新建王国生效档位 = `floor(世界档位 × 王国掌握度)`（最低可为 0/1，按配置）。  
...
> 说明：阈值（10 本/5 城/5000 人等）只是默认值，可在配置中调整。  
...
轮回结算时，系统只做“档位推进”：
- 所有遗产档位 +1（上限 10），不做条件判定。
- 第 1 次完成轮回后解锁第 1 档；第 N 次轮回对应第 N 档（封顶 10）。
...
**效果（默认，示例可配置）**：
- Lv0：无加成
- Lv1：文明全体单位基础属性 +3%（攻击/防御/最大生命（health_max））
- Lv3：基础属性 +10%，并降低“魔王阵营单位生成倍率” 5%（仅影响本 MOD 生成的魔王/将领/军团）
- Lv5：解锁“破魔锻造”：对文明新生/新生成的战士与命定英雄额外施加 `trait_era_sealforged`（攻击/防御/最大生命（health_max） +10%）
...
- 若出现并列：优先“击杀高 / 等级高”，仍相同则在前 3 名里随机。

**命定英雄自带（默认）**：
- 3 个传奇特性（随机选择）
- `trait_era_destined`：基础属性加成（`damage/armor/health/speed/attack_speed/critical_*`，默认值见 7.1，可配置）
...
- 额外加成：`max_mana_bonus`、`combat_mana_regen_bonus`（已在 7.1 里列出默认值）
- 触发时机：仅在“晋升为命定英雄”时一次性写入（不做周期刷新）

**随机特质池（示例，可配置）**：
- 抽取数量：`hero_random_trait_count`（默认 2；可配置）
...
**随机技能池（示例，可配置）**：
- 抽取数量：`hero_random_skill_count`（默认 1；可配置）
...
**模式（可切换/可配置）**：
- **各自征战**：魔王互相中立，只攻击文明。
- **魔王联盟（共同作战全世界）**：魔王互不宣战，统一把文明当敌人。
- **魔王内战**：魔王互相宣战，同时也攻击文明（三方混战）。
- **随机判断**：按“宿敌/相性表”或权重随机决定关系。
- **终极议会（可选）**：达成条件时强制联盟，开启末日倒计时。
...
**魔王内战规则**：
- 魔王阵营两两宣战，同时对文明宣战。
- 默认上限 `5` 个魔王参与“全量互战”，超过则随机抽取对战组合。
- 可选“主导权”机制：击败其他魔王可获得短期加成与称号。

**终极议会（可选）**：
- 触发建议：第 `N` 轮回后 + 存活魔王 ≥ `X` 或 “魔王控制世界 > 50%”。
- 触发后：强制联盟 + 末日倒计时；文明获得短期抗魔加成作为对抗窗口。
...
- 关系维护：每 `T=3-5` 年做一次“关系校验”，按模式修正关系，避免 AI 自发改变。

**可配置参数（默认值示例）**：
- `multi_demon_mode` = `random`（各自征战/联盟/内战/随机/议会）
- `multi_demon_relation_check_years` = 3
- `demon_alliance_strength_mult` = 0.85
- `demon_civilwar_strength_mult` = 1.00
- `demon_council_trigger_cycle` = 5
- `demon_council_control_ratio` = 50%
- `demon_civilwar_max_demons` = 5
...
> 本设计不使用传统冷却条，使用游戏内置“法力（Mana）”作为技能资源，同时采用触发频率限制：普通每年最多 2 次，终极每 2 年最多 1 次（可配置）。
...
> 统一槽位：每个将领 **P0 + S1**；技能触发与性能约束遵循「[文档约定](#conventions)」。
...
- **技能口径**：将领技能只做“强干预”，不承诺接管底层 AI；触发/范围/存储规则见「[文档约定](#conventions)」。
- **持续/限频**：统一写入 `custom_data_*`，并且只在三钩子里检查与更新（例如“每 X 次触发最多 1 次”“最多触发 N 次后失效”）。
- **频率限制**：主动技能默认每年最多 2 次；终极技每 2 年最多 1 次（可配置）。
...
> 表中“实现抓手”是建议路线：优先 `ActionLibrary.*`，没有现成动作时用自定义 trait + `custom_data_*` 存状态，并在三钩子里推进与清理（见「[文档约定](#conventions)」）。
...
| 将领ID | 名称 | 定位 | P0 被动（常驻） | S1 主动（默认消耗 5 点法力） |
|---|---|---|---|---|
...
> 技能触发/范围/存储规则遵循「[文档约定](#conventions)」。

#### 2.7.1 军团通用规则（必须一致）

- **单位分层**：先锋（骚扰）/主力（均衡）/精锐（带技能）/终极（Boss）。
- **同时上限**：受 `2.4` 的“军团_同时上限”约束；超过则停止生成。
- **法力口径**：军团单位若配置 S1 主动技能，则其模板需选用“原生可见法力条”的基底；主动技能消耗法力（全局默认：普通 5 点、终极 10 点），并遵循频率限制（普通每年最多 2 次、终极每 2 年最多 1 次）。
- **技能口径**：军团单位技能尽量短（单次/短持续）；触发与持续清理规则见「[文档约定](#conventions)」。
...
| 单位ID | 名称 | 阶层 | P0 被动（常驻） | S1 主动（默认消耗 5 点法力） |
|---|---|---|---|---|
...
- **建筑/遗迹外观**：自定义 `BuildingAsset` + 自定义贴图与说明；配合小范围地形变化做表现。
...
| 参数 | 默认值 | 备注 |
|------|-----|------|
| `繁荣度_人口阈值` | 3000 | 达成则进入预兆 |
| `繁荣度_城市阈值` | 15 | 达成则进入预兆 |
| `繁荣度_发展阈值` | 5 | 达成则进入预兆 |
| `繁荣度_检查间隔` | 10 年 | 每隔 N 年检查一次繁荣度 |
| `预兆_持续年数` | 30 | 预兆阶段持续 |
| `预兆_事件间隔` | 3-5 年 | 预兆事件触发频率 |
| `苏醒_持续年数` | 50 | 苏醒阶段持续（范围 30-100） |
| `苏醒_初始强度比例` | 30% | 苏醒期魔王强度占最大强度比例 |
| `降临_持续年数_最短` | 100 | 入侵最短 |
| `降临_持续年数_最长` | 200 | 入侵最长 |
| `衰弱_持续年数` | 20 | 衰弱阶段持续 |
| `封印战_持续年数` | 50 | 决战窗口（范围 50-100） |
| `封印_初始强度` | 100% | 可超过 100% |
| `封印_衰减速率` | 5%/10年 | `<= 0` 自动修正 |
| `封印强度_苏醒阈值` | 20% | 低于该值进入苏醒 |
| `封印强度_降临阈值` | 10% | 低于该值进入降临 |

#### 7.1.2 魔王与多魔王

| 参数 | 默认值 | 备注 |
|------|-----|------|
| 魔王启用默认 | 全部启用 | 10 个魔王默认全部勾选 |
| 魔王苏醒模式 | random | 指定/随机 |
| 苏醒数量 | 1 | 仅随机模式生效 |
| 全盛期 HP 阈值 | 70% | >70% 进入全盛 |
| 衰弱期 HP 阈值 | 30% | <30% 进入衰弱 |
| 轮回成长倍率 | 25%/轮回 | 魔王成长倍率 |
| 魔王成长硬上限 | +200% | 约 8 轮回触发 |
| 魔王阵营寿命（lifespan） | 99999 年 | 等效“不老死” |
| 多魔王互动模式 | random | 各自征战/联盟/内战/随机/议会 |
| 多魔王关系校验间隔 | 3 年 | 定期修正关系 |
| 魔王联盟强度系数 | 0.85 | 联盟模式下单魔王强度 |
| 魔王内战强度系数 | 1.00 | 内战模式下单魔王强度 |
| 内战最大魔王数 | 5 | 超过则抽取对战组合 |
| 终极议会触发轮回 | 5 | 可配置 |
| 终极议会触发控制率 | 50% | 魔王控制世界比例阈值 |

#### 7.1.3 军团与波次

| 参数 | 默认值 | 备注 |
|------|-----|------|
| `军团_生成间隔` | 10 年 | 每波间隔（`<= 0` 自动修正） |
| `军团_初始数量` | 30 | 第 1 波数量 |
| `军团_强度递增` | 15%/波 | 每波增长（允许负值） |
| `军团_同时上限` | 200 | 超过则停止生成 |
| `军团_全盛加速` | ×2 | 全盛期生成加速 |
| 波次数量模板 | 1-3 波=30 / 4-6 波=35 / 7-9 波=40 / 10+ 波=50 | 默认模板，可改 |
| 波次等级增量 | 0 / +1 / +2 / +3 | 对应 1-3/4-6/7-9/10+ 波 |
...
|------|-----|------|
| 抗魔等级范围 | 0-10 | 上限 10 |
| 抗魔等级_击杀阈值 | 1000 | 魔王阵营击杀数达标 +1 |
| 抗魔等级_封印加成 | +1.5/次 | 封印战胜利加成 |
| 抗魔等级关键档位效果 | Lv1 +3%；Lv3 +10% 且生成-5%；Lv5 破魔锻造 +10%；Lv7 +25% 且生成-12%；Lv10 +40% 且生成-20% | 默认档位 |
| 命定英雄每文明上限 | 1 名 | 晋升制 |
| 命定英雄晋升冷却 | 30 年 | 英雄死亡后冷却 |
| 英雄候选最低等级 | 3 | 低于该等级不进候选池 |
| 英雄候选最低击杀 | 5 | 低于该击杀不进候选池 |
| 英雄候选最低年龄 | 18 年 | 低于该年龄不进候选池 |
| 英雄评分权重 | level 0.25 / kills 0.25 / life 0.15 / hp 0.10 / atk 0.10 / lead 0.15 | 可改 |
| 英雄评分归一化阈值 | level/10、kills/50、life/200、hp/1000、atk/100 | 可改 |
| 英雄评分取前 N 随机 | 3 | 前 N 名随机 1 名 |
| 命定英雄基础倍率（damage/armor/health/speed） | +35% | `trait_era_destined` 默认值 |
| 命定英雄基础倍率（attack_speed/critical_*） | +20% | `trait_era_destined` 默认值 |
| 命定英雄随机特质数量 | 2 个 | 从“英雄特质池”抽取 |
| 命定英雄随机技能数量 | 1 个 | 从“英雄技能池”抽取 |
| 命定英雄法力上限加成 | +170 | 可配置 |
| 命定英雄战斗回蓝加成 | +1/触发 | 可配置 |
| 魔王阵营法力上限加成 | +0 | 作用于魔王/将领/军团 |
| 魔王阵营战斗回蓝加成 | +0/触发 | 作用于魔王/将领/军团 |
...
| 参数 | 默认值 | 备注 |
|------|-----|------|
| 停战复查间隔 | 3 年 | 建议范围 1-5 年 |
| 共同讨伐触发_城市摧毁比例 | 20% | 魔王摧毁城市比例阈值 |
| 共同讨伐触发_人口阈值 | 500 | 低人口触发，默认沿用人口保护阈值 |

#### 7.1.6 难度与保护（CSI）

| 参数 | 默认值 | 备注 |
|------|-----|------|
| CSI 权重 | 人口/城市/发展/英雄/抗魔 各 25% | 五维平均 |
| CSI 分段阈值 | 30 / 70 | 低/中/高分段 |
| 自适应倍率下限 | 0.85 | CSI < 30 |
| 自适应倍率中段 | 0.90 → 1.10 | CSI 30-70 |
| 自适应倍率上限 | 1.10 | CSI > 70 |
| 倍率变化阻尼 | ±0.05/次 | 平滑波动 |
| 简单判定连胜 | 5 轮 | 未死英雄 + 城市 0 损毁 |
| 简单上调幅度 | +10% | 下轮魔王强度 |
| 保底触发 | 人口 < 500 或 文明 < 2 | 进入保底模式 |
| 保底倍率 | 0.85 | 固定倍率 |
| 无解死档触发 | 存活文明 ≤ 1 | 启用失败不死档 |
| 失败不死档 | 开启 | 可重启轮回保留进度 |
...
| 参数 | 默认值 | 备注 |
|------|-----|------|
| 每魔王将领数量 | 5 名 | 先锋/坦克/输出/辅助/精英 |
| 将领激活数量成长 | 起始 2 / 每轮回 +1 / 上限 6 | 见 3.2 |
| 将领平均等级成长 | 起始 10 / 每轮回 +3 / 上限 40 | 见 3.2 |
| 将领平均属性系数 | 跟随魔王 ×0.7 | 见 3.2 |
| 将领定位倍率模板 | 先锋 0.8/1.0/0.6/1.5；坦克 1.5/0.7/1.5/0.7；输出 0.7/1.5/0.5/1.0；辅助 1.0/0.8/1.0/1.0；精英 1.2/1.2/1.2/1.0 | 顺序=HP/攻/防/速 |
| 军团单位倍率模板 | 先锋 0.7/0.8/0.6/1.3；主力 1.0/1.0/1.0/1.0；精锐 1.6/1.4/1.2/1.0；终极 4.0/2.5/2.0/0.8 | 顺序=HP/攻/防/速 |

#### 7.1.8 技能/遗产/装备（大表可调项）
...
| 军团单位技能表数值 | 见 2.7.4 | 默认值以表内为准 |

#### 7.1.9 体验与显示

| 参数 | 默认值 | 备注 |
|------|-----|------|
| 自动收藏：魔王 | 开启 | 生成时自动加入收藏夹 |
| 自动收藏：将领 | 开启 | 生成时自动加入收藏夹 |
| 自动收藏：命定英雄 | 开启 | 晋升时自动加入收藏夹 |
| 自动收藏：军团 | 关闭 | 默认不收藏，避免刷屏 |

#### 7.1.10 主角单位属性随机范围（生成时一次）
...
- **叠加顺序**：随机倍率视为“基础属性的一部分”，再叠加轮回成长/自适应/特质加成。
- **参数规则**：不设建议上限；`min > max` 自动互换；倍率 `<= 0` 修正为最小正值；`critical_chance` 最终按 `0%-100%` 归一；`NaN/Infinity` 自动回退为安全值并提示。

| 参数 | 默认值 | 备注 |
|------|-----|------|
| `随机倍率_魔王_生命` | 2.00~6.00 | 作用于 `health` |
| `随机倍率_魔王_防御` | 1.80~4.00 | 作用于 `armor` |
| `随机倍率_魔王_伤害` | 2.20~5.00 | 作用于 `damage` |
| `随机加成_魔王_暴击率` | 12%~35% | 作用于 `critical_chance` |
| `随机倍率_魔王_移速` | 1.10~1.40 | 作用于 `speed` |
| `随机倍率_魔王_攻速` | 1.10~1.40 | 作用于 `attack_speed` |
| `随机倍率_将领_生命` | 1.50~3.50 | 作用于 `health` |
| `随机倍率_将领_防御` | 1.30~2.80 | 作用于 `armor` |
| `随机倍率_将领_伤害` | 1.60~3.50 | 作用于 `damage` |
| `随机加成_将领_暴击率` | 10%~25% | 作用于 `critical_chance` |
| `随机倍率_将领_移速` | 1.05~1.25 | 作用于 `speed` |
| `随机倍率_将领_攻速` | 1.05~1.25 | 作用于 `attack_speed` |
| `随机倍率_军团_生命` | 1.20~2.00 | 作用于 `health` |
| `随机倍率_军团_防御` | 1.10~1.80 | 作用于 `armor` |
| `随机倍率_军团_伤害` | 1.20~2.00 | 作用于 `damage` |
| `随机加成_军团_暴击率` | 6%~18% | 作用于 `critical_chance` |
| `随机倍率_军团_移速` | 1.00~1.20 | 作用于 `speed` |
| `随机倍率_军团_攻速` | 1.00~1.20 | 作用于 `attack_speed` |
| `随机倍率_英雄_生命` | 1.60~3.20 | 作用于 `health` |
| `随机倍率_英雄_防御` | 1.30~2.50 | 作用于 `armor` |
| `随机倍率_英雄_伤害` | 1.60~3.00 | 作用于 `damage` |
