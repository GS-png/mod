...
**文档定位**
- 这是“玩法规则 + 可实现口径”的总文档：玩家能快速理解玩法，开发能直接落地实现。
- 玩法/规则在前，技术/API 在后；长表格默认折叠，按需展开。

**读者路线图（按角色）**
- **玩家/体验向**：先看「[1.2 游戏循环流程](#s12)」→「[2.1 轮回系统](#s21)」→「[2.4 敌方军势](#s24)」→「[2.5 文明与英雄系统](#s25)」→「[第四部分：用户交互设计](#p4)」
- **开发/实现向**：先看「[第五部分：技术可行性与实现方案](#p5)」，再回头按需查系统细节
- **只查默认值/参数**：跳到「[7.1 关键参数默认值速查](#s71)」
- **只看技能/单位**：从「[2.2 魔王状态机系统](#s22)」→「[2.4.2 将领系统](#s242)」→「[2.4.3 军团系统](#s243)」进入

**阅读提示**
- 带“实现口径/实现要点/示例代码/【需反射】”的内容偏开发，可按需跳过。
- 想回目录：在任意章节使用「[↩ 目录](#toc)」。
...
阅读提示：带“实现/口径/示例代码/【需反射】”的段落偏开发；玩家可跳过。
...
- **技能口径**：将领技能只做“强干预”，不承诺接管底层 AI；触发/范围/存储规则见「[统一口径](#conventions)」。
