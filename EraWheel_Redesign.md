...
### 4.1 玩家视角的设计目标（干净、清楚、好读）

- **一眼看懂**：打开面板 3 秒内知道“当前轮回/阶段/封印/魔王状态”。  
- **先结论后细节**：先告诉玩家“发生了什么”，再解释“为什么”和“我能做什么”。  
- **术语统一**：同一概念只用一个名字，避免“阶段/时期/进度”混用。  
- **新手友好**：关键名词旁有简短解释（悬浮提示/小问号）。  
- **不打扰**：只对关键事件提醒，细节放到日志。  

### 4.2 双层结构（HUD + 主面板 + 详情抽屉）
...
**HUD 字段说明**  
- **轮回/阶段**：现在处在第几轮、哪一阶段。  
- **封印强度**：越低越接近“封印战”。箭头表示趋势。  
- **魔王**：当前最危险或最活跃的魔王。  
- **文明/人口/英雄**：玩家侧整体生存状态。  
- **警报**：只提示重要事件（城沦陷/魔王苏醒/封印战开启）。  

**B. 主面板（完整交互）**  
通过按钮或快捷键打开，左侧栏目、右侧详情、底部日志。  

```
┌─ 栏目 ─┐┌───────────── 内容区 ─────────────┐┌─ 详情 ─┐
│ 总览   ││ 主要内容卡片/列表/图表           ││ 选中对象│
│ 战局   ││                                   ││ 详细说明│
│ 技能库 ││                                   ││ 规则提示│
│ 封印与遗产│                                   ││ 快捷操作│
│ 玩法百科││                                   ││          │
...
| 栏目 | 你能看到什么 | 你能做什么 |
|---|---|---|
| **总览** | 当前轮回、阶段、封印、魔王与文明概况 | 只读为主，少量快捷按钮 |
| **战局** | 魔王/将领/军团/英雄/文明列表 | 筛选、定位、关注 |
| **技能库** | 所有技能说明与开关 | 搜索、开关、查看详情 |
| **封印与遗产** | 封印进度与遗产选择 | 查看规则、选择遗产 |
| **玩法百科** | 玩法核心说明（新手指南） | 只读学习 |
| **事件与日志** | 事件时间线与故事 | 过滤、导出 |
| **设置** | 参数与预设 | 调参、导入/导出 |
| **调试** | 开发用信息 | 仅开发模式 |
...
- **轮回与封印卡**：轮回数、阶段、封印强度、距离封印战的预计时间  
- **魔王态势卡**：活跃魔王、HP、波次进度、下一次风险  
- **文明概览卡**：文明数量、人口、英雄数、文明强度指数（CSI）  
- **玩法速览卡**：一句话说明 MOD 核心玩法（轮回、魔王、遗产、英雄）  
- **时间线**：最近 6 条大事件  

```
┌─ 轮回与封印 ───────┐  ┌─ 魔王态势 ─────────┐
│ 轮回: 3  阶段: 降临 │  │ 活跃: 虚无之主     │
│ 封印: 18% 下降中    │  │ HP: 80% 波次: 8/15 │
│ 距封印战: 约 40 年  │  │ 预警: 瘟疫母神 22% │
└───────────────────┘  └───────────────────┘

┌─ 文明概览 ─────────┐  ┌─ 玩法速览 ─────────┐
│ 文明: 7  人口: 12.5k│  │ 轮回 + 魔王 + 英雄  │
│ 城市: 45 英雄: 8(2传奇)│  │ 遗产会跨轮回继承 │
│ CSI: 72 (强势)       │  │ 技能可读可控       │
└───────────────────┘  └───────────────────┘
...
- **名称 + 类型**：被动 / 主动  
- **触发方式**：命中 / 受击 / 死亡点  
- **消耗与频率**：消耗多少法力、多久能触发一次  
- **范围与持续**：影响范围、持续时间  
- **效果描述**：用一句话写清“对谁、做什么、持续多久”  
- **限制条件**：阶段限制、目标限制、是否可叠加  
- **开关**：启用 / 禁用（给出“立即生效/下轮回生效”的说明）  
...
### 4.8 玩法百科（新手指南，解释核心玩法）

**这里用“人话”解释 MOD 的核心**：  

1. **轮回是什么**：魔王被封印 → 世界重建 → 下一次魔王再来，这算 1 次轮回。  
2. **阶段怎么走**：预发展 → 预兆 → 苏醒 → 降临 → 衰弱 → 封印战 → 重建。  
3. **玩家要做什么**：发展文明、培养英雄、撑过魔王波次、在封印战击败魔王。  
4. **技能是什么**：魔王/将领/军团/英雄都有技能，技能有触发方式和效果。  
5. **遗产是什么**：每轮回结束会得到永久加成，下一轮回继续生效。  

**三分钟上手清单**  
- 先看 HUD：轮回/阶段/封印是否安全。  
- 打开“总览”：确认文明数量和英雄数量。  
- 打开“技能库”：了解当期魔王最危险的技能。  
- 打开“封印与遗产”：看看下一轮回能拿到哪些遗产。  
...
- **叙事模式**：把事件整理成章节（可展开看原始事件）  
- **安全触发**：只允许触发标记为“安全”的事件  

### 4.10 设置与预设页（玩家能控制的东西）
...
- **颜色**：主色深棕、辅色古金、背景羊皮纸；红色只用于警报。  
- **字体**：标题略古典，正文清晰易读（优先大字号）。  
- **排版**：每屏信息不超过 3 层，列表统一左对齐。  
- **提示**：重要提示用顶部条，避免频繁弹窗。  
- **按钮**：主要按钮高对比，危险按钮二次确认。  
