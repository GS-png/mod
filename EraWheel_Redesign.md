...
**统一口径速览（核心规则）**
- 触发只走三钩子：命中/受击/死亡点；技能不做“每年结算/定时轮询”
- 范围效果只做“有限半径枚举”，禁止全图扫描或维护长期名单
- 主动技能默认用法力，不设传统冷却条；普通 5 点、终极 10 点，且有触发频率限制（普通每年最多 2 次；终极每 2 年最多 1 次，可配置）
- 数值口径统一：百分比、伤害基准、属性字段、HP 口径见下文
- 关键参数全部可配置；非法值自动修正并提示
- 生成与挂载统一：走自定义模板；技能 trait 不进随机池；魔王阵营默认不老死；魔王/将领/军团/命定英雄默认参与原版随机特质（出生/成长）
...
- **模板/法力/频率/触发口径**：统一见「[统一口径](#conventions)」与「[7.1.7](#s71)」，本节不再重复。
...
- 触发只走三钩子（命中/受击/死亡点），不做全图扫描；主动触发频率默认遵循“普通每年 2 次 / 终极每 2 年 1 次”。
...
- **参数规则**：不设建议上限；`min > max` 自动互换；倍率 `<= 0` 修正为最小正值；`critical_chance` 最终按 `0%-100%` 归一；`NaN/Infinity` 自动回退为安全值并提示。
...
- 默认消耗：普通技能 5 点、终极技 10 点；默认频率：普通每年最多 2 次、终极每 2 年最多 1 次；均可配置，可按技能单独覆盖。
...
| S2 | 末日号角 | 主动（命中施放/范围压制） | 命中时尝试；持续 30 年 | 当终焉审判者命中目标且法力足够时：以自身 `pTile` 半径 40 的王国单位获得 30 年“末日压迫”（攻击 -10%，防御 -10%，移速 -5%）。 | 在 `action_attack_target`：`spendMana` 后范围枚举并给目标加 `trait_era_doom_pressure`（到期年）。不做“定时全图触发”。 |
