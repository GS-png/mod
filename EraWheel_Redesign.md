...
- **开发/实现向**：先看「[附录A：统一口径](#conventions)」→「[附录B：技术可行性与实现方案](#p5)」，再回头按需查系统细节
...
- 需要实现时只看「[附录A：统一口径](#conventions)」+「[附录B：技术可行性与实现方案](#p5)」和文中“实现口径/示例代码/【需反射】”标记段。
- 只改默认值时直接去「[7.1 关键参数默认值速查](#s71)」（附录D）。
- 想回目录：在任意章节使用「[↩ 目录](#toc)」。
...
文明通过与魔王战斗，逐步提升抗魔等级（上限可配置，默认值见 [7.1.4](#s71)），形成“越战越强”的正反馈。  
...
默认阈值/权重/倍率见 [7.1.4](#s71)，本节只讲规则与口径。

**产生规则（晋升制，默认）**：
- 每个文明在每个轮回内有“命定英雄上限”（默认值见 [7.1.4](#s71)）。
- 当文明满足繁荣/危机阈值（人口、城市、战损等）时，系统从该文明现存单位中挑选 1 名候选并“晋升”为命定英雄（阈值见 [7.1.4](#s71)）：
  - 优先：高等级/高击杀/寿命时间长/基础属性高的单位
  - 若没有合格候选：允许直接生成 1 名“命定英雄模板单位”（可选）

**评分规则（可配置）**：
- **候选池过滤**：同文明、已成年、存活、非魔王/将领/军团、未被标记为英雄（阈值见 [7.1.4](#s71)）。
- **评分指标**：等级、击杀、寿命（或最大寿命）、最大生命、攻击、领导力（声望/指挥/组织）。
- **归一化**：将各指标归一化到 0-1（阈值见 [7.1.4](#s71)）。
- **加权求和**：按权重得到综合分（权重见 [7.1.4](#s71)）。
- **晋升选择**：默认从前 N 名中随机 1 名，避免总是同一个人（N 见 [7.1.4](#s71)）。
- **并列处理**：优先“击杀高 / 等级高”，仍相同则在前 N 名中随机。
- **兼容性**：若某项数据缺失，则忽略该项并把权重平均分给其他项，避免“没数据就被扣分”。

**命定英雄自带**：
- `trait_era_destined`：基础属性加成（默认值见 [7.1.4](#s71)）
...
- 额外特质/技能按配置抽取（数量与规则见 [7.1.4](#s71)）

**命定英雄强化参数（可配置）**：
- 基础倍率：`damage/armor/health/speed/attack_speed/critical_chance/critical_damage_multiplier`（默认值见 [7.1.4](#s71)）
- 额外加成：`max_mana_bonus`、`combat_mana_regen_bonus`（默认值见 [7.1.4](#s71)）
- 触发时机：仅在“晋升为命定英雄”时一次性写入（不做周期刷新）

**随机特质池（示例，可配置）**：
- 抽取数量：`hero_random_trait_count`（默认值见 [7.1.4](#s71)）
...
**随机技能池（示例，可配置）**：
- 抽取数量：`hero_random_skill_count`（默认值见 [7.1.4](#s71)）
...
**家族继承**：
- 英雄死亡后，后代有一定概率继承部分能力（默认值见 [7.1.4](#s71)）
- 可形成"英雄家族"，延续故事线
...
- 英雄权重 = 传奇英雄数 / 世界英雄总数
- 抗魔权重 = 抗魔等级 / 抗魔等级上限（默认值见 [7.1.4](#s71)）
...
| **过于困难** | 人口/文明低于阈值（见 [7.1.6](#s71)） | 自动进入"保底模式"（倍率见 [7.1.6](#s71)） |
...
- [ ] 人口/文明触发保底阈值见 [7.1.6](#s71)，不可继续提升难度
