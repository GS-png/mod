...
**文档定位**
- 这是“玩法规则 + 可实现口径”的总文档：玩家能一眼看懂玩法，开发能直接落地实现。
- 玩法/规则在前，技术/API 在后；长表格用 `<details>` 折叠，按需展开。
...
**阅读方式（避免跳来跳去）**
- 先读「[1.2 游戏循环流程](#s12)」+「[2.1 轮回系统](#s21)」，再读「[2.2-2.5](#p2)」补齐魔王/军势/英雄的关系。
- 需要实现时只看「[第五部分：技术可行性与实现方案](#p5)」和文中“实现口径/示例代码/【需反射】”标记段。
- 只改默认值时直接去「[7.1 关键参数默认值速查](#s71)」。
- 想回目录：在任意章节使用「[↩ 目录](#toc)」。
