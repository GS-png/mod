...
**持续规则**：
- 系统自动生成封印遗迹（自定义建筑）与“仪式区域”（范围检测 + 进度条逻辑）；遗迹数量可配置（见 [7.1.12](#s71)）
...
- 可通过两种方式之一胜利：
  ...
  2. **仪式封印**：友方单位在遗迹范围内完成“占据”（先清理完遗迹范围内的魔王势力，且人数达到阈值）即可**触发占领状态**；触发后不要求持续保持人数，除非触发中断条件。  
     ...
     （占领人数与判定方式/清场要求/占领遗迹数量/进度速度/中断条件可配置，见 [7.1.12](#s71)）
  3. **遗迹争夺驱动（让双方会去打）**：
...
**结束条件**：
- 满足任意胜利条件 → 魔王被再封印（胜利条件可配置，见 [7.1.12](#s71)）
- 或魔王恢复→回到降临阶段
...
- **人口灭绝保护**：世界人口 < `人口保护阈值` 时，魔王强度按 `人口保护倍率` 下调（默认 50%），给文明喘息机会（见 [7.1.6](#s71)）
- **文明灭绝保护**：存活文明 ≤ `文明保护阈值` 时，自动开启“失败不死档”（阈值/开关可配置，见 [7.1.6](#s71)）
- **失败不死档**：文明全灭后，可选"重启轮回"，保留轮回数+部分遗产，从预发展阶段重新开始
- **封印胜利保底**：可配置是否启用“击杀封印”保底规则（见 [7.1.12](#s71)）
...
- 新建王国生效档位 = `floor(世界档位 × 王国掌握度)`（最低档位可配置，见 [7.1.11](#s71)）。  
...
> 说明：阈值与权重只是默认值，可在配置中调整（见 [7.1.11](#s71)）。  
...
- 运行中：可选每 `掌握度刷新间隔` 年更新新王国的掌握度（默认 10 年，见 [7.1.11](#s71)；不建议每年重算）。  
...
轮回结算时，系统只做“档位推进”：
- 所有遗产档位 +1（上限默认 10），不做条件判定（上限/步进可配置，见 [7.1.11](#s71)）。
...
- 若开启“遗迹战场热点”，波次会优先将集结目标指向“未占领/被敌占领”的遗迹，保证战斗集中（见 [7.1.12](#s71)）
...
- **模板/法力/频率/触发口径**：统一见「[统一口径](#conventions)」与「[7.1.8](#s71)」，本节不再重复。
...
- Lv0：无加成
- Lv1：文明全体单位基础属性小幅提升  
- Lv3：基础属性进一步提升，并降低“魔王阵营单位生成倍率”（仅影响本 MOD 生成的魔王/将领/军团）
- Lv5：解锁“破魔锻造”（对新生战士/命定英雄追加 trait）
- Lv7：大幅提升，并继续降低“魔王阵营单位生成倍率”
- Lv10：顶级加成，达到最高抑制幅度
...
- 当文明满足繁荣/危机阈值（人口、城市、战损等，**任一满足即可**；**统计范围=该文明自身**）时，系统从该文明现存单位中挑选 1 名候选并“晋升”为命定英雄：
...
**模式（可切换/可配置）**：
- **各自征战**：魔王互相中立，但都攻击所有非魔王阵营。
- **魔王联盟（共同作战全世界）**：魔王互不宣战，统一把所有非魔王阵营当敌人。
- **魔王内战**：魔王互相宣战，同时也攻击所有非魔王阵营（三方混战）。
...
**终极议会（可选）**：
- 触发建议：满足配置条件（轮回阈值/控制率阈值见 [7.1.2](#s71)）。
...
- 字段清单：`multi_demon_mode`、`multi_demon_relation_check_years`、`demon_alliance_strength_mult`、`demon_civilwar_strength_mult`、`demon_council_trigger_cycle`、`demon_council_control_ratio`、`demon_civilwar_max_demons`。
...
> 本设计不使用传统冷却条，使用游戏内置“法力（Mana）”作为技能资源；触发频率统一规则见「[统一口径](#conventions)」与「[7.1.8](#s71)」。
...
说明：以下 S1 消耗与频率遵循统一口径（见 [7.1.8](#s71)），不再逐条重复标注。
...
- 人口权重 = 当前人口 / 最大历史人口
- 城市权重 = 当前城市 / 历史最多城市
- 发展权重 = 发展等级 / 发展等级上限（发展等级定义见 2.1.2）
- 英雄权重 = 传奇英雄数 / 世界英雄总数
- 抗魔权重 = 抗魔等级 / 抗魔等级上限
...
自适应倍率计算：
- CSI 分为低/中/高三段（阈值见 [7.1.6](#s71)）
- 低段/高段使用固定倍率，中段线性插值（倍率见 [7.1.6](#s71)）

变化阻尼：
- 单次变化幅度有上限（默认步长见 [7.1.6](#s71)）
- 避免忽强忽弱的体验问题
```
...
| 场景 | 触发条件 | 反应 |
|------|--------|------|
| **过于简单** | 连续胜利且损失很低（条件见 [7.1.6](#s71)） | 下轮魔王强度上调（幅度见 [7.1.6](#s71)） |
| **过于困难** | 人口/文明低于阈值（见 [7.1.6](#s71)） | 触发人口/文明保护（倍率见 [7.1.6](#s71)） |
| **无解死档** | 存活文明过少（见 [7.1.6](#s71)） | 开启"失败不死档"，可重启轮回保留进度 |
...
- [ ] CSI 自适应倍率变化幅度有上限（默认步长见 [7.1.6](#s71)）
- [ ] 高轮回（5+）存在至少 3 种不同的克制策略
- [ ] 人口/文明保护阈值见 [7.1.6](#s71)，不可继续提升难度
...
| 参数 | 默认值 | 备注 |
|------|-----|------|
| `世界现存人口阈值` | 5000 | 用于检测进入下一阶段（预发展） |
| `世界现存城市阈值` | 20 | 用于检测进入下一阶段（预发展） |
| `预发展_检查间隔` | 10 年 | 每隔 N 年检查一次 |
| `预兆_持续年数` | 30 | 预兆阶段持续 |
...
| `衰弱_持续年数` | 20 | 衰弱阶段持续 |
| `封印战_持续年数` | 50 | 决战窗口 |
| `封印战_回退_HP阈值` | 70% | 魔王恢复到该阈值以上则回到降临 |
| `封印_初始强度` | 100% | 可超过 100% |
...
| 全盛期法力恢复倍率 | +50% | 全盛期法力恢复加成 |
| 衰弱期将领数量倍率 | 0.5 | 衰弱期将领数量乘数 |
| 轮回成长倍率 | 25%/轮回 | 魔王成长倍率 |
...
| 命定英雄兜底检测间隔 | 5 年 | 仅检测是否满足阈值，不强制晋升 |
| 命定英雄晋升_繁荣_人口阈值 | 3000 | 统计范围=该文明；繁荣条件任一满足即可 |
| 命定英雄晋升_繁荣_城市阈值 | 15 | 统计范围=该文明；繁荣条件任一满足即可 |
| 命定英雄晋升_繁荣_发展阈值 | 5 | 发展等级 0-10；统计范围=该文明 |
| 命定英雄晋升_危机统计窗口 | 10 年 | 统计范围=该文明；近 N 年 |
| 命定英雄晋升_危机_人口跌幅 | 20% | 统计范围=该文明；危机条件任一满足即可 |
| 命定英雄晋升_危机_城市损失数 | 2 | 统计范围=该文明；危机条件任一满足即可 |
| 命定英雄晋升_危机_战损比例 | 30% | 统计范围=该文明；危机条件任一满足即可 |
...
| 共同讨伐触发_降临阶段开关 | 开启 | 开启则魔王进入「降临」即可触发停战 |
...
| 简单上调幅度 | +10% | 下轮魔王强度 |
| 人口保护阈值 | 500 | 人口 < 阈值触发人口灭绝保护 |
| 人口保护倍率 | 0.50 | 触发后魔王强度乘数 |
| 文明保护阈值 | 1 | 存活文明 ≤ 阈值触发文明灭绝保护 |
| 失败不死档 | 开启 | 触发后可重启轮回保留进度 |
...
#### 7.1.12 封印遗迹与仪式封印（UI 可调）

| 参数 | 默认值 | 备注 |
|------|-----|------|
| 封印遗迹数量 | 10 | 每次封印战生成数量 |
| 占领遗迹数量封印阈值 | 8 | 需同时占领的遗迹数量 |
| 仪式占领_人数阈值 | 20 | 遗迹范围内友军人数 |
| 仪式占领_人数判定 | 触发式 | 达标即开始，不需持续保持 |
| 仪式占领_清场要求 | 开启 | 需清理遗迹范围内魔王势力后才可开始 |
| 仪式进度速度 | 10%/年 | 进度累积速度 |
| 仪式中断条件 | 敌军进入 | 可选：敌军进入/敌军攻击/任一 |
| 仪式中断处理 | 暂停 | 可选：暂停/回退/清零 |
| 仪式回退速度 | 5%/年 | 仅“回退/清零”时生效 |
| 封印胜利模式 | 双通道 | 可选：仅击杀/仅仪式/双通道 |
| 封印胜利保底 | 开启 | 关闭时不强制“击杀封印”保底 |
| 遗迹战场热点 | 开启 | 是否定期派小队去争夺遗迹 |
| 热点调度间隔 | 5 年 | 每次派遣间隔 |
| 热点派遣_文明人数 | 30 | 从附近文明抽取的单位数 |
| 热点派遣_魔王人数 | 30 | 从魔王阵营抽取的单位数 |
| 热点目标策略 | 优先敌占领 | 可选：最近/随机/优先敌占领 |
| 遗迹占领收益_半径 | 12 | 仅遗迹范围内生效 |
| 遗迹占领收益_友军增益 | 攻防 +10% | 占领方范围增益 |
| 遗迹占领收益_敌军削弱 | 攻防 -10% | 非占领方范围减益 |
| 遗迹占领收益_叠加上限 | 1 | 同阵营收益最多叠加层数 |
