...
public class MapGenTemplateLibrary : AssetLibrary<MapGenTemplate>
{
	...
	public void createDefaultBackupValues()
	{
		...
		{
			MapGenValues value = JsonUtility.FromJson<MapGenValues>(JsonUtility.ToJson(item.values));
	...
	{
		MapGenValues values = JsonUtility.FromJson<MapGenValues>(JsonUtility.ToJson(default_values[pAsset.id]));
