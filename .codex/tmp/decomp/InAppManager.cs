...
public class InAppManager : MonoBehaviour, IDetailedStoreListener, IStoreListener
{
	...
	public void checkPremium()
	{
		...
		Debug.Log("[cp]");
		Debug.Log("[avl] " + product.availableToPurchase);
		Debug.Log("[txi] " + product.transactionID);
		Debug.Log("[hrc] " + product.hasReceipt);
		...
		Debug.Log("[vp] 3 " + flag);
		Debug.Log("[hr] " + product.hasReceipt);
		if ((flag && product.hasReceipt) || PlayerConfig.instance.data.premium)
		{
			Debug.Log("[phr]");
			activatePrem();
		}
	...
	public PurchaseProcessingResult ProcessPremium(PurchaseEventArgs args)
	{
		...
		string text = args?.purchasedProduct?.definition?.id;
		Debug.Log("[vp] 2 " + flag);
		Debug.Log("[lc] " + text);
