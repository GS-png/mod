...
public class BuildingDebugAnimationElement : BaseDebugAnimationElement<BuildingAsset>
{
	...
	public override void setData(BuildingAsset pAsset)
	{
		...
		{
			BuildingDebugAnimationVariation buildingDebugAnimationVariation = UnityEngine.Object.Instantiate(variation_prefab, variations_transform);
	...
	public void setFrames(List<DebugAnimatedVariation> pVariations, bool pShouldHaveSprites)
	{
		...
		for (int i = 0; i < pVariations.Count; i++)
		{
			...
			{
				buildingDebugAnimationVariation.image.color = Color.clear;
				buildingDebugAnimationVariation.shadow.color = Color.clear;
				continue;
			}
			...
			if (frames == null || frames.Length == 0)
			{
				...
				buildingDebugAnimationVariation.shadow.color = Color.clear;
				buildingDebugAnimationVariation.enabled = false;
			...
			if (!debugAnimatedVariation.animated)
			{
				...
				buildingDebugAnimationVariation.image.sprite = sprite;
				if (asset.shadow)
				{
					DynamicSpriteCreator.createBuildingShadow(asset, sprite, pIsContructionSprite: false);
					buildingDebugAnimationVariation.shadow.sprite = DynamicSprites.getShadowBuilding(asset, sprite);
				}
				else
				{
					buildingDebugAnimationVariation.shadow.color = Color.clear;
			...
			Sprite[] array = new Sprite[frames.Length];
