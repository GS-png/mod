...
public class ComponentListBase<TListElement, TMetaObject, TData, TComponent> : MonoBehaviour, IComponentList, IShouldRefreshWindow where TListElement : WindowListElementBase<TMetaObject, TData> where TMetaObject : CoreSystemObject<TData> where TData : BaseSystemData where TComponent : ComponentListBase<TListElement, TMetaObject, TData, TComponent>
{
	...
	private void LateUpdate()
	{
		...
		float y = scroll_rect.content.localPosition.y;
		float pScrollRectTop = y + scroll_rect.viewport.rect.height;
	...
	{
		BaseEmptyListMono original = Resources.Load<BaseEmptyListMono>("ui/list_element_empty");
		original = UnityEngine.Object.Instantiate(original, list_transform);
		original.gameObject.SetActive(value: false);
		if (_element_height > 0f && original.TryGetComponent<LayoutElement>(out var component))
		{
			component.minHeight = _element_height;
		}
		_pool_empty_elements = new ObjectPoolGenericMono<BaseEmptyListMono>(original, list_transform);
	}
