...
public class NameplateText : MonoBehaviour
{
	...
	internal void showTextAlliance(Alliance pMetaObject, City pCity)
	{
		...
		setText(text, pCity.city_center);
		setPriority(num);
	...
	internal void showTextClanCity(Clan pMetaObject, City pCity)
	{
		...
		int count = pMetaObject.units.Count;
		string stringForNameplate = getStringForNameplate(pMetaObject.name, count);
		setText(stringForNameplate, pCity.city_center);
		setPriority(count);
	...
	internal void showTextCity(City pMetaObject, Vector2 pPosition)
	{
		...
		{
			Vector2 anchoredPosition = ((!is_full) ? new Vector2(3f, -25f) : new Vector2(0f, -1f));
	...
	{
		Vector2 vector = transformPosition(pPos);
		if (vector != _last_position)
		{
			_last_position = vector;
			base.transform.position = vector;
			recalcScaledOverlapRect(_rect_background, ref map_text_rect_click);
			float num = map_text_rect_click.width * 0f * 0.5f;
			float num2 = map_text_rect_click.height * 0f * 0.5f;
			map_text_rect_overlap.x = map_text_rect_click.x + num;
			map_text_rect_overlap.y = map_text_rect_click.y + num2;
			map_text_rect_overlap.width = map_text_rect_click.width - num * 2f;
			map_text_rect_overlap.height = map_text_rect_click.height - num2 * 2f;
		}
	}
	...
	private void recalcScaledOverlapRectCorners(RectTransform pRectBackground, ref Rect pMapTextRect)
	{
		...
		pMapTextRect.x = num;
		pMapTextRect.y = num3;
		pMapTextRect.width = num2 - num;
		pMapTextRect.height = num4 - num3;
	}
	...
	private void recalcScaledOverlapRectSimple(RectTransform pRectBackground, ref Rect pMapTextRect)
	{
		...
		if (is_mini)
		{
			sizeDelta.Set(60f, 60f);
		}
		float num = sizeDelta.x * 0.55f * cached_canvas_scale;
		float num2 = sizeDelta.y * 0.55f * cached_canvas_scale;
		Vector3 position = pRectBackground.position;
		pMapTextRect.x = position.x - num * 0.5f;
		pMapTextRect.y = position.y - num2 * 0.5f;
		pMapTextRect.width = num;
		pMapTextRect.height = num2;
	}
	...
	public void setAssetAndMeta(NameplateAsset pAsset, NanoObject pNano)
	{
		...
		if (_asset != pAsset)
		{
			...
			RectOffset padding = layout_group.padding;
			if (is_mini)
			{
				padding.left = 0;
				padding.right = 0;
