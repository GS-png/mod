...
public class HistoryDataLibrary : AssetLibrary<HistoryDataAsset>
{
	...
	public override void post_init()
	{
		...
		foreach (HistoryDataAsset item in list)
		{
			Color color = Toolbox.makeColor(item.color_hex);
			if (color.grayscale < 0.28f)
			{
				Color color2 = Color.Lerp(color, Color.white, 0.33f);
				string tooltip_color_hex = "#" + ColorUtility.ToHtmlStringRGB(color2);
