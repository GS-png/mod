...
public class StatsWindow : TabbedWindow
{
	...
	internal StatsRowsContainer stats_rows_container
	{
		get
		{
			if (_stats_rows_container == null)
			{
				_stats_rows_container = base.transform.FindRecursive("content_stats").GetComponent<StatsRowsContainer>();
				if (_stats_rows_container == null)
				{
	...
	{
		base.create();
		_refreshable_elements = GetComponentsInChildren<IRefreshElement>(includeInactive: true);
	}
	...
	{
		stats_rows_container.gameObject.SetActive(value: false);
		stats_rows_container.gameObject.SetActive(value: true);
	}
