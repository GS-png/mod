...
public class GridLayoutGroupExtended : LayoutGroupExtended
{
	...
	private void TweenLayoutInputForAxis(float totalMin, float totalPreferred, float totalFlexible, int axis)
	{
		...
		Vector3 vector = new Vector3(totalMin, totalPreferred, totalFlexible);
		Vector3 vector2 = new Vector3(GetTotalMinSize(axis), GetTotalPreferredSize(axis), GetTotalFlexibleSize(axis));
		if (vector == vector2)
		{
			return;
		}
		TweenerCore<Vector3, Vector3, VectorOptions> tweenerCore = _axis_tween[axis];
		if (tweenerCore.IsActive())
		{
			if (tweenerCore.endValue == vector)
			{
				return;
			}
			tweenerCore.Kill();
		}
		_axis_tween[axis] = DOPreferredSize(vector, moveDuration * 0.5f, axis);
	}

	private TweenerCore<Vector3, Vector3, VectorOptions> DOPreferredSize(Vector3 endValue, float duration, int axis)
	{
		TweenerCore<Vector3, Vector3, VectorOptions> tweenerCore = DOTween.To(() => new Vector3(GetTotalMinSize(axis), GetTotalPreferredSize(axis), GetTotalFlexibleSize(axis)), delegate(Vector3 x)
		{
			SetLayoutInputForAxis(x.x, x.y, x.z, axis);
		}, endValue, duration);
