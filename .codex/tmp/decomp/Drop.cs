...
public class Drop : BaseMapObject
{
	...
	{
		Vector2 vector = Randy.randomPointOnCircle(pMinRadius, pMaxRadius);
	...
	{
		float z = base.transform.rotation.eulerAngles.z + _rotation_speed * pElapsed;
	...
	private void updateParabolicFall(float pElapsed)
	{
		if (!(_timeInAir > _timeToTarget))
		{
			...
			Vector2 vector = Toolbox.ParabolaDrag(_startPosition, _targetPosition, _targetHeight, num);
			Vector2 vector2 = Vector2.Lerp(_startPosition, _targetPosition, num);
			_currentHeightZ = vector.y - vector2.y;
			float x = vector.x;
			float newY = vector.y - _currentHeightZ;
			current_position.Set(x, newY);
	...
	{
		Vector3 position = new Vector3(current_position.x, current_position.y + _currentHeightZ, _currentHeightZ);
	...
	private void OnDrawGizmos()
	{
		if (_parabolic && !_landed && _timeToTarget != 0f && !(_timeInAir > _timeToTarget))
		{
			...
			Vector2 vector2 = _startPosition;
			int num = 60;
			for (int i = 1; i <= num; i++)
			{
				float pTime = (float)i / (float)num * _timeToTarget;
				Vector2 vector3 = Toolbox.ParabolaDrag(_startPosition, _targetPosition, _targetHeight, pTime);
				Vector2 vector4 = Toolbox.Parabola(_startPosition, _targetPosition, _targetHeight, pTime);
				Gizmos.color = _gizmoColor;
