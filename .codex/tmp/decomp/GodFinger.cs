...
public class GodFinger : BaseActorComponent
{
	...
	public override void update(float pElapsed)
	{
		...
		fingerTip.gameObject.SetActive(flag);
		if (flag)
		{
	...
	public static void debug_trail(GodFinger pFinger)
	{
		...
		if (ai.hasTask())
		{
			...
			case "godfinger_circle_move_small":
				color_white = Toolbox.color_fire;
				break;
			}
			BaseEffect baseEffect = EffectsLibrary.spawn("fx_weapon_particle", null, null, null, 0f, pFinger.fingerTip.transform.position.x, pFinger.fingerTip.transform.position.y);
			if (baseEffect != null)
			{
				((StatusParticle)baseEffect).spawnParticle(baseEffect.transform.position, color_white);
			}
		}
	}
}
