...
public class LoadingScreen : MonoBehaviour
{
	...
	private void setupBg()
	{
		...
		if (lastBgHeight != num2 || lastBgWidth != num || canvas.scaleFactor != lastCScale)
		{
			...
			float num3 = (float)background.mainTexture.width * canvas.scaleFactor;
			float num4 = (float)background.mainTexture.height * canvas.scaleFactor;
			float num5 = (float)Screen.width / num3;
			float num6 = (float)Screen.height / num4;
			if (num5 > num6)
			{
				background.transform.localScale = new Vector3(num5, num5, 1f);
			}
			else
			{
				background.transform.localScale = new Vector3(num6, num6, 1f);
			}
		}
	}
	...
	private void Update()
	{
		...
		float num = bar.transform.localScale.x;
		if (bar.transform.localScale.x < asyncLoad.progress)
		{
			num = bar.transform.localScale.x + Time.deltaTime * 2f;
			if (num > asyncLoad.progress)
			{
				num = asyncLoad.progress;
			}
			bar.transform.localScale = new Vector3(num, 1f, 1f);
		}
