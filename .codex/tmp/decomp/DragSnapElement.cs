...
public class DragSnapElement : MonoBehaviour, IInitializePotentialDragHandler, IEventSystemHandler, IDragHandler, IBeginDragHandler, IEndDragHandler, IScrollHandler, IDraggable, IPointerEnterHandler, IPointerExitHandler
{
	...
	private void Start()
	{
		...
		if (base.gameObject.TryGetComponent<ScrollableButton>(out var component))
		{
	...
	public float getDragMod()
	{
		...
		float num = Mathf.Clamp01((vector - base.transform.position).magnitude / limit_max_drag_distance);
	...
	{
		Vector3 vector = _start_parent.TransformPoint(_start_local_position);
		Vector3 vector2 = pTargetPosition - vector;
		if (vector2.magnitude > limit_max_drag_distance)
		{
			base.transform.position = vector + vector2.normalized * limit_max_drag_distance;
		}
		else
		{
	...
	public void OnEndDrag(PointerEventData pEventData)
	{
		if (Config.isDraggingItem() && Config.isDraggingObject(this))
		{
			...
			}, vector, num).SetEase(ease).OnComplete(resetElement);
	...
	{
		float t = (Mathf.Clamp(pDistance, snapback_min_distance, snapback_max_distance) - snapback_min_distance) / (snapback_max_distance - snapback_min_distance);
		return Mathf.Lerp(snapback_speed_min_distance, snapback_speed_max_distance, t);
	}
