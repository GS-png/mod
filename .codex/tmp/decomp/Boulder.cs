...
public class Boulder : BaseEffect
{
	...
	private void updateForce(float pElapsed)
	{
		...
		Vector2 vector = calcCurrentPos();
		setCurrentPosition(vector.x, vector.y, heightPosition);
	...
	private void updateShadow()
	{
		...
		float shadowAlpha = Mathf.Clamp(value, 0.15f, 1f) * 0.3f;
		setShadowAlpha(shadowAlpha);
		float num = Mathf.Clamp(value, 0.25f, 0.9f) * 0.3f;
		Vector3 localScale = shadowTransform.localScale;
		localScale.Set(num, num, 1f);
	...
	public void spawnOn(Vector2 pPosition)
	{
		...
		if (isRandomLaunch(pPosition))
		{
			vector.x = Randy.randomFloat(-40f, 40f);
			vector.y = Randy.randomFloat(-40f, 40f);
			vector = Vector2.ClampMagnitude(vector, 40f);
		}
		else
		{
			vector = chargeVector(pPosition) * 0.777f;
		}
		_previous_bounce_position = pPosition;
		_previous_bounce_position.y -= getHeightPosition();
		_previous_bounce_position -= vector * getBounceProgress();
		for (int i = 0; i < 3; i++)
		{
			int num = i + 1;
			Vector2 item = new Vector2
			{
				x = _previous_bounce_position.x + vector.x * (float)num,
				y = _previous_bounce_position.y + vector.y * (float)num
			};
