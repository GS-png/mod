...
public class DragOrderElement : MonoBehaviour, IDraggable, IEndDragHandler, IEventSystemHandler
{
	...
	{
		Transform parent = main_transform.parent;
	...
	{
		Vector2 vector = getChildPositionInContainer();
		if (!((Vector2)main_transform.localPosition == vector) && !(current_destination == vector))
		{
	...
	private void moveDraggingTab()
	{
		...
		case DragOrderContainer.SnapAxis.Horizontal:
			mousePosition.y = main_transform.position.y;
			break;
		case DragOrderContainer.SnapAxis.Vertical:
			mousePosition.x = main_transform.position.x;
			break;
		}
		if (!_container.limit_moving)
		{
			main_transform.position = mousePosition;
			return;
		}
		Rect worldRect = _container.rect_transform.GetWorldRect();
		getGridValues(_container.grid_layout, out var pCellSize, out var _);
		pCellSize *= 0.5f;
		mousePosition.x = Mathf.Min(mousePosition.x, worldRect.xMax - pCellSize.x);
		mousePosition.x = Mathf.Max(mousePosition.x, worldRect.xMin + pCellSize.x);
		mousePosition.y = Mathf.Min(mousePosition.y, worldRect.yMax - pCellSize.y);
		mousePosition.y = Mathf.Max(mousePosition.y, worldRect.yMin + pCellSize.y);
		main_transform.position = mousePosition;
	}
	...
	private void endDrag()
	{
		if (Config.isDraggingObject(this))
		{
			...
			current_destination = childPositionInContainer;
			is_target_reached = false;
			if (!(_container.dragging_element != this))
			{
	...
	{
		Vector2 point = _container.rect_transform.InverseTransformPoint(Input.mousePosition);
	...
	public Rect getRect()
	{
		...
		Vector2 position = (Vector2)main_transform.localPosition - pCellSize * main_transform.pivot - pSpacing / 2f;
		Vector2 size = pCellSize + pSpacing;
		return new Rect(position, size);
	}

	private void getGridValues(LayoutGroup pLayoutGroup, out Vector2 pCellSize, out Vector2 pSpacing)
	{
		if (!(pLayoutGroup is GridLayoutGroup gridLayoutGroup))
		{
	...
	private static bool shouldStartDrag(Vector2 pPressPos, Vector2 pCurrentPos)
	{
		...
		return (pPressPos - pCurrentPos).sqrMagnitude >= num * num;
	}
