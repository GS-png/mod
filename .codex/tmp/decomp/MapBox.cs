...
public class MapBox : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		canvas = transform.FindRecursive("Canvas - UI/General").GetComponent<Canvas>();
		transition_screen = transform.GetComponentInChildren<LoadingScreen>(includeInactive: true);
		console = transform.GetComponentInChildren<WorldBoxConsole.Console>(includeInactive: true);
		nameplate_manager = transform.GetComponentInChildren<NameplateManager>(includeInactive: true);
		tutorial = transform.GetComponentInChildren<Tutorial>(includeInactive: true);
		selected_buttons = transform.GetComponentInChildren<PowerButtonSelector>();
	...
	private void Start()
	{
		...
		if (Config.isComputer || Config.isEditor)
		{
			...
			_unit_select_effect = UnityEngine.Object.Instantiate(original, base.gameObject.transform).GetComponent<UnitSelectionEffect>();
	...
	public static AttackDataResult checkAttackFor(AttackData pData, BaseSimObject pTargetToCheck)
	{
		...
		float num = Toolbox.SquaredDist(target.x, target.y + pTargetToCheck.getHeight(), pData.hit_position.x, pData.hit_position.y + pData.hit_position.z);
		float num2 = pData.area_of_effect + pTargetToCheck.stats["size"];
		num2 *= num2;
		if (num < num2)
		{
			Vector3 vector = Vector3.MoveTowards(pData.hit_position, target, pTargetToCheck.stats["size"] * 0.9f);
			vector.y += pTargetToCheck.getHeight();
	...
	private void debugHouses()
	{
		foreach (Building building in buildings)
		{
			...
			{
				Debug.LogError("Building " + building.data.id + " has residents but is not usable");
			}
			if (!building.asset.docks && building.countResidents() > building.asset.housing_slots)
			{
				Debug.LogError(building.asset.id + " has more residents than housing " + building.countResidents() + "/" + building.asset.housing_slots);
			}
		}
	}
	...
	public void locateSelectedVillage()
	{
		...
		move_camera.focusOn(selected_city.city_center);
	}
