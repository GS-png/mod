...
public class DraggableLayoutElement : MonoBehaviour, IInitializePotentialDragHandler, IEventSystemHandler, IDragHandler, IBeginDragHandler, IEndDragHandler, ILayoutIgnorer, IDraggable
{
	...
	public void Update()
	{
		if (_dragging_cache != _any_dragging)
		{
			...
			foreach (MonoBehaviour toggle_element in _toggle_elements)
			{
				...
				{
					(toggle_element as Selectable).interactable = !_any_dragging;
				}
				else
				{
					toggle_element.enabled = !_any_dragging;
				}
			}
		}
		moveToTarget();
	...
	private void recalcParent()
	{
		if (!(_cached_parent_position == _parent.position))
		{
			...
			float num = _rect.rect.width * 10f;
			float num2 = _rect.rect.height * 10f;
			_cached_parent_rect.x -= num;
			_cached_parent_rect.y -= num2;
			_cached_parent_rect.width += num * 2f;
			_cached_parent_rect.height += num2 * 2f;
		}
	}
	...
	private void findTarget()
	{
		...
		Vector3 position = _drag_object.transform.position;
		...
		{
			float num4 = Vector2.Distance(_sibling_positions[i], position);
		...
		if (num2 != num3 && !Mathf.Approximately(a, num))
		{
			_target_index = num2;
		}
	}
	...
	private void swapSiblings(int pStartIndex, int pTargetIndex)
	{
		...
		{
			int siblingIndex = _siblings[pStartIndex].transform.GetSiblingIndex();
			int siblingIndex2 = _siblings[pTargetIndex].transform.GetSiblingIndex();
			if (siblingIndex > siblingIndex2)
			{
				_siblings[pTargetIndex].transform.SetSiblingIndex(siblingIndex);
				base.transform.SetSiblingIndex(siblingIndex2);
			}
			else
			{
				base.transform.SetSiblingIndex(siblingIndex2);
	...
	{
		int result = pIndex;
		Vector2 b = _sibling_positions[pIndex];
		float num = float.MaxValue;
		foreach (ref int pNeighbour in pNeighbours)
		{
			int current = pNeighbour;
			float num2 = Vector2.Distance(_sibling_positions[current], b);
			if (num2 < num)
			{
				num = num2;
				result = current;
	...
	private ListPool<int> getNeighbours(int pIndex)
	{
		...
		Vector2 a = _sibling_positions[pIndex];
		float num = Vector2.Distance(_sibling_positions[0], _sibling_positions[1]) * 1.5f;
		for (int i = 0; i < _sibling_positions.Length; i++)
		{
			if (i != pIndex && Vector2.Distance(a, _sibling_positions[i]) <= num)
			{
				listPool.Add(i);
			}
		}
