...
public class GenericCustomizeWindow<TMetaObject, TData, TBanner> : MonoBehaviour where TMetaObject : MetaObject<TData> where TData : MetaObjectData where TBanner : BannerGeneric<TMetaObject, TData>
{
	...
	protected virtual void updateColorsBanner()
	{
		...
		image_banner_option_1.color = color.getColorMainSecond();
		if (meta_asset.option_2_color_editable)
		{
			image_banner_option_2.color = color.getColorBanner();
		}
	}
	...
	{
		Transform transform = base.transform.FindRecursive("Background").Find("Title");
		title = transform.GetComponent<LocalizedText>();
		...
		Transform pTransform = base.transform.FindRecursive("Banner");
		banner_area = pTransform.FindRecursive("BannerArea");
		icon_banner = pTransform.FindRecursive("Icon").GetComponent<Image>();
	...
	protected virtual void setupButtons()
	{
		...
		ColorElement color_element_prefab = GetComponentInParent<CustomizeWindow>().color_element_prefab;
		for (int i = 0; i < meta_asset.color_count(); i++)
		{
			...
			Color pInner = colorByIndex.getColorBorderInsideAlpha32();
			ColorElement tColorElement = UnityEngine.Object.Instantiate(color_element_prefab, colors_parent);
	...
	protected virtual void setupBanner()
	{
		...
		banner = UnityEngine.Object.Instantiate(val);
