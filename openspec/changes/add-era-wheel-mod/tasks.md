...
### 1.7 失败保护
- [x] 1.7.1 实现失败条件检测
- [x] 1.7.2 实现"终末余波"分支（部分完成：目前主要是日志分支，缺少明确的游戏内效果/提示）
  - [x] 1.7.2.a 设计并实现至少1个“终末余波”的可见效果（例如持续惩罚、世界事件、明显提示）
  - [x] 1.7.2.b 在UI/通知中提示玩家已进入“终末余波”
- [x] 1.7.3 实现"重启轮回"功能（部分完成：已能强制重启，但缺少玩家选择/确认流程）
  - [x] 1.7.3.a 在失败触发时弹出选择：重启轮回/继续终末余波（或仅确认重启）
  - [x] 1.7.3.b 重启轮回后状态/遗产保留比例在UI里有明确提示
- [x] 1.7.4 实现遗产保留比例配置
...
- [x] 1.9.2 实现 `Code/UI/Panels/OverviewPanel.cs` 总览面板（部分完成：数据与文本已具备，但面板结构仍偏占位）
  - [x] 1.9.2.a 面板显示/隐藏与刷新行为在游戏内可验证
- [x] 1.9.3 实现 `Code/UI/Panels/DemonPanel.cs` 魔王面板（简化版）（部分完成：数据与操作已具备，但面板结构仍偏占位）
  - [x] 1.9.3.a 可在游戏内查看魔王列表与详情，并执行“强制苏醒/启用禁用”等基础操作
- [x] 1.9.4 实现 `Code/UI/NotificationSystem.cs` 通知系统（部分完成：目前以日志为主，缺少可见提示）
  - [x] 1.9.4.a 在屏幕上显示通知（不依赖打开主面板）
  - [x] 1.9.4.b 通知数量/持续时间受配置控制
...
- [x] 2.1.2 实现死亡君王·阿努比斯
- [x] 2.1.3 实现机械暴君·欧米茄
- [x] 2.1.4 实现灵魂编织者·墨菲斯托
- [x] 2.1.5 实现轮回解锁节奏配置

### 2.2 将领系统
- [x] 2.2.1 实现 `Code/DemonLords/General/BaseGeneral.cs` 将领基类
- [x] 2.2.2 实现将领AI行为树
- [x] 2.2.3 实现将领技能系统
- [x] 2.2.4 实现将领背叛机制
